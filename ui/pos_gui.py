...
# 新しい支払いロジック関数
from logic.payment_manager import (
    get_initial_amount,
    process_payment,
    get_payments_summary,
    reset_payments  # 事前に payment_manager.py に実装してある想定です
)
...
class POSApp:
    ...
    def setup_ui(self):
        ...
        self.code_entry.bind("<Return>", lambda e: (play_beep(), self.search_product(auto_register=True)))
        ...
        tk.Label(frame, textvariable=self.suggestion_var, fg="gray").grid(row=0, column=2, sticky="w")
        ...
        # 日次処理実行ボタン
        self.daily_btn = tk.Button(
            frame, text="日次処理実行", command=self.run_daily_tasks)
        self.daily_btn.grid(row=6, column=0,
                            columnspan=len(buttons), pady=10)
    ...
    def update_suggestion(self):
        ...
        self.suggestion_var.set(
            f"候補: {product.get('goods_name','')}" if product else "")
    ...
    def register_product(self):
        ...
        self.listbox.insert(
            tk.END,
            f"{display_name}  ¥{int(price_value)}円"
        )
        self.update_total()
    ...
    def handle_item_fixed_discount(self):
        ...
        # カートインデックス算出
        item_index = sum(
            1 for j in range(idx+1)
            if not self.listbox.get(j).startswith('  ')
        ) - 1
    ...
    def handle_item_percent_discount(self):
        ...
        pct = ask_price(
            self.root,
            title='個別％割引', prompt='割引率(％)を入力してください：'
        )
        if pct is None:
            return
    ...
    def open_payment_popup(self, total_due):
        ...
        for m in ["現金", "クレカ", "QR"]:
            tk.Button(popup, text=m, command=lambda m=m: _process(m))\
              .pack(side="left", padx=5)
        tk.Button(popup, text="キャンセル", command=popup.destroy)\
          .pack(side="left", padx=5)
    ...
    def complete_payment(self, total_due):
        ...
        payments = [
            {"method": m, "amount": a}
            for m, a in summary.items() if a > 0
        ]
        ...
        self.discount_manager.order_discounts.clear()
        # 支払い累計もリセット
        self.payment_pm.reset()

    #――――― 入出金 ―――――
    def handle_deposit(self):
        amt = ask_price(self.root,
                        title='入金額入力',
                        prompt='現金入金額を入力してください：')
        if amt is None:
            return
        path = self.cf_recorder.record_deposit(amount=amt)
        messagebox.showinfo(
            '入金完了',
            f"{int(amt):,} 円を入金として記録しました。\n{path} へ保存されました。"
        )

    def handle_withdraw(self):
        amt = ask_price(self.root,
                        title='出金額入力',
                        prompt='現金出金額を入力してください：')
        if amt is None:
            return
        path = self.cf_recorder.record_withdraw(amount=amt)
        messagebox.showinfo(
            '出金完了',
            f"{int(amt):,} 円を出金として記録しました。\n{path} へ保存されました。"
        )
