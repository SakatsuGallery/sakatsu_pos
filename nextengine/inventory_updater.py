...
class InventoryUpdater:
    ...
    def __init__(self, token_env=".env", simulate=True):
        ...
        # Store tokens in attributes that won't clash with methods
        ...
        with open(self.token_env, 'w', encoding='utf-8') as f:
            for line in lines:
                if line.startswith('NE_ACCESS_TOKEN='):
                    f.write(f"NE_ACCESS_TOKEN={data['access_token']}\n")
                elif line.startswith('NE_REFRESH_TOKEN='):
                    f.write(f"NE_REFRESH_TOKEN={data['refresh_token']}\n")
                else:
                    f.write(line)

        # Update in-memory tokens
        self.ne_access_token = data['access_token']
        self.ne_refresh_token = data['refresh_token']
    ...
    def update_from_record(self, json_path):
        ...
        dest = os.path.join('data', 'success') if result and result.get('result') == 'success' else os.path.join('data','pending')
        ensure_dir(dest)
        shutil.move(json_path, os.path.join(dest, os.path.basename(json_path)))
